...
def convert_and_extend(
    ...
    if file_ext == '.pkl':
        ...
        if isinstance(loaded_data, dict):
            # 如果是字典，直接使用
            data = loaded_data
        elif isinstance(loaded_data, np.lib.npyio.NpzFile):
            # 如果是 npz 对象，转换为字典
            data = {key: loaded_data[key] for key in loaded_data.keys()}
        else:
            # 尝试转换为字典（假设是包含所需键的对象）
            try:
                data = dict(loaded_data)
            except:
                raise ValueError(f"无法解析 .pkl 文件格式。期望字典或包含 'fps', 'root_pos', 'root_rot', 'dof_pos', 'dof_vel' 的对象")
    elif file_ext == '.npz':
        # 加载 npz 文件
        data = np.load(input_file, allow_pickle=True)
    else:
        raise ValueError(f"不支持的文件格式: {file_ext}。仅支持 .npz 和 .pkl 文件")
    ...
    if isinstance(fps_value, np.ndarray):
        fps = int(fps_value[0]) if fps_value.shape else int(fps_value)
    else:
        fps = int(fps_value)
    ...
    dof_vel = np.array(data["dof_vel"])  # 新增
...
def batch_convert_and_extend(
    ...
    if pattern == "*.{npz,pkl}":
        ...
        input_files = sorted(npz_files + pkl_files)
    else:
        # 使用原始模式
...
注意:
  此版本需要输入数据包含 dof_vel 字段，直接使用而不是通过有限差分计算。
    ...
    parser.add_argument(
        "--pattern",
        default="*.{npz,pkl}",
        help="批量模式下的文件匹配模式 (default: *.{npz,pkl}，支持 .npz 和 .pkl 文件)",
    )
    parser.add_argument(
        "--visualize",
        "-v",
